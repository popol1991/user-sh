...
// internal commands that execute in child process
char* _INT_CMD[4] = {"cd", "jobs", "history", "pwd"};
...
void exec_command(command cmd) {
    ...
    if (pid < 0) {
        ...
        if (is_bg) {
            // ensure that child processs won't end 
            // before parent add it into job list
            while (goon == 0);
            goon = 0;
        }
		...
		// parent process
        // add the child pid into front pid list
...
void waitchild() {
    ...
    int count = 0;
    if (is_bg) {
        // add background job into job list
        // signal child process to continue after that
...
void execute() {
    ...
    // iterate all command if there's a pipeline
    while (p) {
        count++;
        cmd = p->cmd;
        ...
        // fg, bg and cd have to be executed in the parent process
